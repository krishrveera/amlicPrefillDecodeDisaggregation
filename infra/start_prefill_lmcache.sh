#!/usr/bin/env bash
# =============================================================================
# start_prefill_lmcache.sh — Run vLLM as KV producer using LMCacheConnectorV1
# Run on: L4 VM (PREFILL VM)
# Requires: Redis running on this VM (run start_redis.sh first)
# =============================================================================
set -euo pipefail
...
REDIS_HOST="${REDIS_HOST:-${VM_PREFILL_IP:-127.0.0.1}}"
REDIS_PORT="${REDIS_PORT:-6379}"
HF_CACHE="${HF_CACHE:-$HOME/.cache/huggingface}"
CONTAINER_NAME="amlic-prefill"

echo "============================================"
echo "  Starting PREFILL container (LMCache KV producer)"
...
sudo docker run -d --name "$CONTAINER_NAME" \
    ...
    -e HF_TOKEN="$HF_TOKEN" \
    -e LMCACHE_CONFIG_FILE="/configs/lmcache-prefiller-config.yaml" \
    ...
    -c "pip install lmcache -q && python -m vllm.entrypoints.openai.api_server \
        --model $MODEL \
        --host 0.0.0.0 \
        --port $PORT \
