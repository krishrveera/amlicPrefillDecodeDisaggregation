...
PROMPTS = [
    ...
    {
        "label": "P6_long",
        "text": "You are a world-class expert in computer architecture, distributed systems, machine learning infrastructure, and GPU programming. I need you to write an exhaustive technical treatise covering the following topics with extreme depth and precision. First, explain the von Neumann bottleneck and how it manifests in modern GPU architectures, covering memory hierarchies from L1 cache through L2 cache through HBM through DRAM, the specific bandwidth numbers for H100 NVLink versus PCIe versus DDR5, and why the memory wall problem is fundamentally worse for transformer inference than for training workloads. Second, derive from first principles the arithmetic intensity of the prefill phase versus the decode phase in transformer inference, showing the exact FLOP counts for a single attention layer with sequence length S, hidden dimension D, and number of heads H, and explain why prefill is compute-bound while decode is memory-bandwidth-bound. Third, explain PagedAttention in vLLM, how it manages KV cache memory using fixed-size pages analogous to virtual memory, why this eliminates memory fragmentation, and how continuous batching builds on top of PagedAttention to improve GPU utilization. Fourth, explain the NIXL library architecture in detail, how it uses UCX as the underlying transport layer, what the compatibility hash validation does and why it fails for heterogeneous GPU architectures with different attention backends, and what would need to change in the library to support heterogeneous deployments. Fifth, compare and contrast the KV cache transfer approaches used in production disaggregated serving systems: NVLink-based transfer for same-node GPUs, RDMA over InfiniBand for same-datacenter GPUs, TCP over Ethernet for cross-datacenter GPUs, and application-layer serialization via Redis for cross-account cloud deployments. For each approach give the theoretical bandwidth, typical latency, and the minimum prompt length at which disaggregation becomes beneficial given that transfer overhead.",
    },
    {
        "label": "P7_vlong",
        "text": "You are simultaneously a professor of computer science specializing in distributed systems, a GPU architect at NVIDIA, a researcher at a leading AI lab working on LLM inference optimization, and a cloud infrastructure engineer who has deployed large language models at scale. I need you to write the most comprehensive possible technical document covering every aspect of the following interconnected topics. Begin with the history of parallel computing from Flynn taxonomy through SIMD through GPGPU through modern tensor cores, explaining how each architectural innovation changed the computational profile of deep learning workloads and why transformer models in particular benefit from the specific hardware characteristics of modern GPUs. Then derive the complete mathematical framework for transformer attention from first principles, starting from the original Bahdanau attention mechanism through scaled dot-product attention through multi-head attention through grouped query attention, showing the exact memory access patterns for each variant and how they determine the operational intensity in FLOPS per byte of memory bandwidth consumed. Then explain in complete detail why the prefill phase of autoregressive language model inference has fundamentally different computational characteristics from the decode phase, deriving the exact arithmetic intensity for each phase as a function of sequence length batch size model dimension and number of attention heads, and showing at what parameter values each phase transitions from compute-bound to memory-bandwidth-bound operation according to the roofline model. Then explain the complete design of the vLLM serving system including PagedAttention KV cache management continuous batching chunked prefill scheduling preemption handling multi-GPU tensor parallelism and pipeline parallelism, and how each of these design decisions affects the tradeoff between time to first token and inter-token latency for different workload distributions. Then explain the disaggregated prefill-decode architecture as proposed in DistServe and Splitwise, including the complete mathematical model for when disaggregation is beneficial as a function of prompt length output length batch size network bandwidth and transfer latency, and derive the crossover threshold formula N star equals the square root of transfer overhead divided by the per-token prefill compute saving. Then explain the engineering challenges of implementing KV cache transfer between GPU nodes including serialization format choices memory layout compatibility issues between different attention backend implementations attention to NIXL UCX and why NIXL fails for heterogeneous GPU architectures that use different attention kernels, and what a heterogeneous compatible transfer module would need to implement including precision alignment VRAM layout normalization and parallel strategy alignment as described in recent literature. Then explain the LMCache architecture in complete detail including its chunked transfer approach the local CPU buffer design the Redis remote backend the connector plugin interface for vLLM and why the shared cache approach using Redis provides a viable alternative to direct NIXL transfer for heterogeneous or cross-account cloud deployments at the cost of increased transfer latency. Finally synthesize all of the above into a complete performance model for a disaggregated LLM serving system deployed across two L4 GPUs connected via a Redis shared cache over a WireGuard VPN tunnel with 50 millisecond round trip latency, deriving the expected TTFT as a function of prompt length and showing the theoretical crossover threshold beyond which the disaggregated system outperforms a collocated baseline on a single L4 GPU.",
    },
]
